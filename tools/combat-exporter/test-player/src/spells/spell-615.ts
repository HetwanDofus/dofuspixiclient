...
export class Spell615 extends BaseSpell {
  ...
  private spawnPierres(particleSystem: ASParticleSystem, _anchor: { x: number; y: number }): void {
  ...
  update(deltaTime: number): void {
    ...
    if (this.mainAnim.isStopped() && !this.particles1.hasAliveParticles() && !this.particles2.hasAliveParticles()) {
      this.complete();
    }
  }
