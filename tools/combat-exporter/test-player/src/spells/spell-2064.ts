...
export class Spell2064 extends BaseSpell {
  ...
  protected setup(context: SpellContext, textures: SpellTextureProvider, init: SpellInitContext): void {
    ...
    const dy = (context?.cellTo?.y ?? 0) - 40 - ((context?.cellFrom?.y ?? 0) - 40);
    ...
    if (beamNativeWidth > 0) {
      this.beamAnim.sprite.scale.x = (targetWidth / beamNativeWidth) * init.scale;
    ...
    this.clip1Anim = this.anims.add(new FrameAnimatedSprite({
    ...
    const impactY = ((context?.cellTo?.y ?? 0) - 40 - (context?.cellFrom?.y ?? 0)) * init.scale;
