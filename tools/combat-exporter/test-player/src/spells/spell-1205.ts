...
export class Spell1205 extends BaseSpell {
  ...
  update(deltaTime: number): void {
    ...
    const sprite6ParticlesArr = (this.sprite6Particles as unknown as { particles: Array<{ alive: boolean; x: number; y: number; t: number; sprite: { position: { set: (x: number, y: number) => void }; scale: { set: (s: number) => void }; rotation: number } }> }).particles;
    for (let i = 0; i < this.sprite6States.length; i++) {
      ...
      const vx = Math.abs(state.v * Math.cos(state.angle * 0.017453292519943295));
    ...
    const sprite4ParticlesArr = (this.sprite4Particles as unknown as { particles: Array<{ alive: boolean; x: number; y: number; t: number; sprite: { position: { set: (x: number, y: number) => void }; scale: { set: (sx: number, sy: number) => void }; rotation: number } }> }).particles;
