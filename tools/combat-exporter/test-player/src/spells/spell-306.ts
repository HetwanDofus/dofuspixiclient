...
export class Spell306 extends BaseSpell {
  ...
  private spawnPierre(): void {
    ...
    this.particles.spawn({
      ...
      vr: 0,
      vrDecay: 1,
      t: 100 * (1 / Math.max(-pierresAnchor.x * PIERRES_MANIFEST.width / PIERRES_MANIFEST.width, 0.01)),
      vt: 0,
      vtDecay: 0,
      rotation,
      alpha: 1,
  ...
  update(deltaTime: number): void {
    ...
    if (!this.spawningDone) {
      ...
      while (this.particleSpawnFrameAccum >= this.particleFrameTime && this.spawnCount < this.maxSpawns) {
        this.spawnCount += 1;
        this.spawnPierre();
