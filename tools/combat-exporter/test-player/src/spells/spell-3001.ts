...
export class Spell3001 extends BaseSpell {
  ...
  protected setup(context: SpellContext, textures: SpellTextureProvider, init: SpellInitContext): void {
    ...
    // AS frame_1: random rotation, scale 0-60%, random offset ±10px, gotoAndPlay(random(5)+1)
    {
      ...
      this.sprite26Anim = this.anims.add(new FrameAnimatedSprite({
    ...
    // offsetY = -17, so position it at targetX, targetY - 17*scale
    {
      ...
      this.sprite29Anim = this.anims.add(new FrameAnimatedSprite({
    ...
    //   for each active element: spawn n particles
    {
      ...
      const params = context?.params ?? { fire: false, water: false, earth: false, air: false };
  ...
  update(deltaTime: number): void {
    ...
    this.sprite4Vr *= Math.pow(0.9, frameDelta);
