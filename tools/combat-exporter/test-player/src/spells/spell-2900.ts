...
export class Spell2900 extends BaseSpell {
  ...
  private spawnFeuxInstances(textures: SpellTextureProvider, init: SpellInitContext): void {
    if (this.feuxSpawned) {
      return;
    }
    ...
    for (let i = 0; i < feuxCount; i++) {
      ...
      const feux = this.anims.add(new FrameAnimatedSprite({
      ...
      feux.sprite.rotation = ((-20 + Math.floor(Math.random() * 40)) * Math.PI) / 180;
    ...
    for (let i = 0; i < minifeux3Count; i++) {
      const spark = this.anims.add(new FrameAnimatedSprite({
