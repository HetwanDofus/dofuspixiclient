...
export class Spell816 extends BaseSpell {
  ...
  protected setup(context: SpellContext, textures: SpellTextureProvider, init: SpellInitContext): void {
    ...
    const levelFrameIndex = Math.max(0, Math.min(level - 1, anim1Textures.length - 1));
    const anchor1 = calculateAnchor(ANIM1_MANIFEST);

    // We use a FrameAnimatedSprite but immediately stop at the level frame
    const levelAnim = this.anims.add(new FrameAnimatedSprite({
    ...
    const rotatedAnim = this.anims.add(new FrameAnimatedSprite({
      textures: anim1Textures,
      anchorX: anchor1.x,
      anchorY: anchor1.y,
      scale: init.scale,
    ...
    const sprite7 = this.anims.add(new FrameAnimatedSprite({
      textures: anim1Textures,
      anchorX: anchor1.x,
      anchorY: anchor1.y,
      scale: init.scale,
    ...
    const sprite10 = this.anims.add(new FrameAnimatedSprite({
      textures: anim1Textures,
      anchorX: anchor1.x,
      anchorY: anchor1.y,
      scale: init.scale,
    ...
    const sprite11 = this.anims.add(new FrameAnimatedSprite({
      textures: anim1Textures,
      anchorX: anchor1.x,
      anchorY: anchor1.y,
      scale: init.scale,
