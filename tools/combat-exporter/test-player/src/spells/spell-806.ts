 ...
 */
function computeSprite6ScalesAtFrames(parentT: number, frameCount: number): number[] {
...
export class Spell806 extends BaseSpell {
  ...
  // Separate tracking for completion frames
  private sprite7Complete = false;
  private sprite8Complete = false;
  private sprite9Complete = false;
  private sprite10Complete = false;
  private sprite11Complete = false;
  ...
  protected setup(context: SpellContext, textures: SpellTextureProvider, init: SpellInitContext): void {
    ...
    const bgAnim = this.anims.add(new FrameAnimatedSprite({
    ...
    this.sprite6Anim = this.anims.add(new FrameAnimatedSprite({
      textures: anim1Textures,
      anchorX: anim1Anchor.x,
      anchorY: anim1Anchor.y,
      scale: init.scale,
    ...
    // DefineSprite_7: Ring effect, t=7, hit at frame 22, remove at frame 91
    this.sprite7Anim = this.anims.add(new FrameAnimatedSprite({
      textures: anim1Textures,
      anchorX: anim1Anchor.x,
      anchorY: anim1Anchor.y,
      scale: init.scale,
    ...
    this.sprite7Anim
      ...
      .onFrame(21, () => { // frame 22 = index 21
        this.signalHit();
      });
    this.targetContainer.addChild(this.sprite7Anim.sprite);

    // DefineSprite_8: Ring effect, t=11, hit at frame 64, remove at frame 106
    this.sprite8Anim = this.anims.add(new FrameAnimatedSprite({
      textures: anim1Textures,
      anchorX: anim1Anchor.x,
      anchorY: anim1Anchor.y,
      scale: init.scale,
  ...
  update(deltaTime: number): void {
    ...
    if (!this.sprite6Stopped) {
      ...
      while (this.sprite6FrameAccum >= this.FRAME_TIME && !this.sprite6Stopped) {
        ...
        if (this.sprite6Frame >= 18) { // stopped at frame 19 (index 18)
