...
export class Spell201 extends BaseSpell {
  ...
  private spawnGriffe(yOffset: number, startFrame: number, rotationDeg: number, _depth: number): GriffeInstance {
  ...
  update(deltaTime: number): void {
    ...
    if (!this.launcherStopped) {
      ...
      while (this.launcherAccum >= this.launcherFrameTime && !this.launcherStopped) {
    ...
    for (let i = 0; i < this.griffeInstances.length; i++) {
      ...
      if (i === 0) {
        ...
        if (inst.anim.getFrame() === 0) {
          inst.anim.sprite.rotation = ((Math.floor(Math.random() * 90) - 45) * Math.PI) / 180;
        }
      } else if (i === 1) {
        // PlaceObject2_6_1 (spawned second, rotation = random(90) + 135)
        if (inst.anim.getFrame() === 0) {
          inst.anim.sprite.rotation = ((Math.floor(Math.random() * 90) + 135) * Math.PI) / 180;
        }
      }
