...
export class FrameAtlas {
  ...
  init(): boolean {
    console.log(
      `[FrameAtlas] ${this.atlasW}x${this.atlasH} atlas, slot=${this.slotW}, ${this.maxSlots} slots, res=${this.resolution}`
    );
    let result: { texture: GPUTexture; textureId: number } | null;

    try {
      result = this.vello.createAtlas(
        this.atlasW,
        this.atlasH
      ) as typeof result;
    } catch (e) {
  ...
  getFrame(
    ...
    const colorsArg = colors
      ? new Uint32Array([colors[0], colors[1], colors[2]])
      : undefined;
    ...
    try {
      dims = this.vello.queueFrame(
        ...
        this.resolution,
        colorsArg,
        accInfo ? new Uint32Array(accInfo) : undefined,
        px,
        py
      ) as typeof dims;
    } catch (e) {
  ...
  flush(): void {
    ...
    try {
      this.vello.flushFrames(this.atlasTextureId);
    } catch {}
  ...
  private evictLRU(): number {
    ...
    for (const [key, slot] of this.frameCache) {
      if (slot.lastUsedTick >= this.currentTick) {
        continue;
      }
    ...
    slot.pixiTexture.frame.copyFrom(new Rectangle(0, 0, 0, 0));
    slot.pixiTexture.updateUvs();
