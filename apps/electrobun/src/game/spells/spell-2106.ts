...
export class Spell2106 extends BaseSpell {
  ...
  update(deltaTime: number): void {
    ...
    this.moveI += 0.6;
    this.moveContainer.rotation =
      ((90 + this.moveA * Math.cos(this.moveI)) * Math.PI) / 180;
    this.moveA /= 1.1;
    ...
    if (!this.shootAnim.isComplete()) {
      this.impactI += Math.PI;
      this.impactContainer.rotation =
        ((90 + this.impactA * Math.cos(this.impactI)) * Math.PI) / 180;
      this.impactA /= 1.5;
    }
