...
export class Spell401 extends BaseSpell {
  ...
  private updatePoints(): void {
    for (const particle of this.points) {
      ...
      const p = particle as PointParticle & {
        rx: number;
        ry: number;
        currentX: number;
        currentY: number;
      ...
      p.anim.sprite.position.set(
        p.currentX * this.initScale,
        p.currentY * this.initScale
      );
