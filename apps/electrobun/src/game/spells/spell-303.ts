...
export class Spell303 extends BaseSpell {
  ...
  update(deltaTime: number): void {
    ...
    if (this.mainAnim.isStopped() || this.mainAnim.isComplete()) {
      if (
        !this.pierresParticles1.hasAliveParticles() &&
        !this.pierresParticles2.hasAliveParticles()
      ) {
        this.complete();
      }
    }
