...
const NAMES = [
  "Xelor-du-Temps", "Iop-le-Brave", "Cra-la-Flèche", "Sadida-Vert",
  "Eniripsa-Douce", "Ecaflip-Lucky", "Osamodas-Fou", "Sram-Ombre",
  "Féca-Shield", "Sacrieur-Blood", "Pandawa-Ivre", "Enutrof-Gold",
];
...
/** Interface for the battlefield methods the stress test needs */
export interface StressTestBattlefield {
  addWorldActor(data: {
    id: number;
    name: string;
    cellId: number;
...
export class StressTest {
  private battlefield: StressTestBattlefield;
  ...
  constructor(
    battlefield: StressTestBattlefield,
    mapWidth: number,
    mapHeight: number,
    cells: CellData[]
  ) {
  ...
  private moveRandomly(actor: StressActor): void {
    ...
    actor.moving = true;
    this.battlefield.moveWorldActor(actor.id, truncated).then(() => {
  ...
  stop(): void {
    ...
    for (const actor of this.actors) {
      ...
      this.battlefield.removeWorldActor(actor.id);
    }
    this.actors = [];
