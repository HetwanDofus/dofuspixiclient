...
export function usePixiSlot(
  factory: (app: Application, container: Container) => (() => void) | void,
  deps: DependencyList = []
...
export function usePixiSlotShared(
  ...
  factory: (container: Container) => (() => void) | void,
  deps: DependencyList = []
): RefObject<HTMLDivElement | null> {
  const ref = useRef<HTMLDivElement | null>(null);

  useEffect(() => {
    if (!mainApp || !ref.current) return;
  ...
  }, [mainApp, ...deps]); // eslint-disable-line react-hooks/exhaustive-deps
