  ...
  return (
    <Panel title="Amis" width={280} height={368} onClose={onClose} zoom={zoom}>
      <div style={{ position: "relative", width: "100%", height: "100%", fontSize: p(11) }}>
        ...
        <div style={{ position: "absolute", left: 0, top: 0, width: "100%", height: tabBarH, display: "flex", borderBottom: `${p(1)}px solid var(--dofus-bar-border, #514a3c)` }}>
          {["friends", "enemies", "ignored"].map((tab) => (
            <button
              ...
              style={{
                flex: 1,
                background: activeTab === tab ? "var(--dofus-header-bg, #514a3c)" : "transparent",
                color: activeTab === tab ? "var(--dofus-text-white, #ffffff)" : "var(--dofus-text-dark, #514a3c)",
                border: "none",
            ...
            >
              {tab === "friends" ? "Amis" : tab === "enemies" ? "Ennemis" : "Ignorés"}
            </button>
          ))}
        </div>
        ...
        >
          <div style={{ fontSize: p(10), fontWeight: "bold", marginBottom: p(8) }}>
            Ajouter un ami
          </div>
