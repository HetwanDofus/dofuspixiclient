...
export function GuildPanel({ onClose, zoom = 1 }: GuildPanelProps) {
  ...
  const TAB_NAMES = ["Membres", "Infos", "Bonus", "Percepteurs", "Enclos", "Maisons"];
  const TAB_KEYS = ["members", "info", "bonus", "perceptors", "enclos", "houses"];

  return (
    <Panel title="Guilde" width={320} height={363} onClose={onClose} zoom={zoom}>
      <div style={{ position: "relative", width: "100%", height: "100%", overflow: "hidden" }}>
        ...
        >
          {TAB_KEYS.map((key, idx) => (
            <button
              ...
              style={{
                flex: 1,
                background: activeTab === key ? COLORS.HEADER_BG : COLORS.BG_ALT,
        ...
        <div style={{ position: "relative", top: p(20), width: "100%", height: "100%" }}>
          ...
          {activeTab !== "members" && (
            ...
            >
              {activeTab === "info" ? "Infos" : activeTab === "bonus" ? "Bonus" : activeTab === "perceptors" ? "Percepteurs" : activeTab === "enclos" ? "Enclos" : "Maisons"}
            </div>
          )}
        </div>
      </div>
