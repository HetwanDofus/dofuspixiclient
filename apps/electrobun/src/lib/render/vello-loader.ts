 ...
 */
export async function loadDofAsset(
  id: number,
  url: string
): Promise<boolean> {
  if (!renderer) throw new Error("Vello not initialized");
 ...
 */
export function loadDofAssetFromBytes(
  id: number,
  data: Uint8Array
): boolean {
  if (!renderer) throw new Error("Vello not initialized");
...
export function renderFrame(
  ...
  const result = renderer.renderFrame(
    assetId, animation, frameIndex, resolution,
    colors ?? undefined,
...
export function getAnimationInfo(
  ...
  if (!renderer) return null;
  return renderer.getAnimationInfo(assetId, animation) as VelloAnimationInfo | null;
}
